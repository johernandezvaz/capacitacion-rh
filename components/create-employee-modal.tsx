...
export function CreateEmployeeModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (data.code === '23505' || data.error?.includes('ya existe')) {
          setErrors({ employee_number: 'Este número de empleado ya existe' });
        } else {
          toast({
            title: 'Error',
            description: data.error || 'No se pudo crear el empleado',
            variant: 'destructive',
          });
        }
        return;
      ...
      setFormData({ employee_number: '', nombre: '', area: '', puesto: '', evaluador: '' });
      onSuccess(data.employee.id);
    } catch (error) {
      toast({
        title: 'Error',
