...
export function DeteccionFormModal({ open, onOpenChange, onSaved, plantId, yearId, editingId, editingData }: Props) {
  ...
  const loadAllEmps = async () => {
    try {
      const res = await fetch(`/employees/data?plant_id=${plantId}`, { credentials: 'include' });
      if (res.ok) {
        const json = await res.json();
        const mapped: EmpOption[] = (json.employees || []).map((e: any) => ({
          id: e.id,
          nombre: e.nombre,
          puesto: e.puesto,
  ...
  const searchSuggestions = async (tabId: string, term: string) => {
    try {
      const res = await fetch(`/detecciones/data?plantId=${plantId}&yearId=${yearId}`);
      if (res.ok) {
        const json = await res.json();
        const matches = (json.detecciones || [])
          .filter((d: any) => d.nombre.toLowerCase().includes(term.toLowerCase()))
          .slice(0, 5)
          .map((d: any) => ({ id: d.id, nombre: d.nombre, color: d.color }));
        updateTab(tabId, { suggestions: matches, showSuggestions: matches.length > 0 });
      }
    } catch (err) {
  ...
  const searchCourses = async (tabId: string, term: string) => {
    try {
      ...
      if (res.ok) {
        const json = await res.json();
        const matches = (json.courses || [])
          .filter((c: any) => c.name.toLowerCase().includes(term.toLowerCase()))
          .slice(0, 8)
          .map((c: any) => ({ id: c.id, name: c.name, year: json.year?.year ?? null }));
        updateTab(tabId, { courseSuggestions: matches, showCourseSuggestions: matches.length > 0 });
      }
    } catch (err) {
  ...
  const handleSelectSuggestion = async (tabId: string, sug: Suggestion) => {
    ...
    try {
      ...
      if (!res.ok) return;
      const json = await res.json();
      const det = (json.detecciones || []).find((d: any) => d.id === sug.id);
      if (!det) return;

      const empLinks = (json.employeeLinks || []).filter((l: any) => l.deteccion_id === sug.id);
      const empIds = empLinks.map((l: any) => l.employee_id);

      updateTab(tabId, {
        nombre: det.nombre || '',
        inst_interno: !!det.inst_interno,
        inst_externo: !!det.inst_externo,
        proveedor_sugerido: det.proveedor_sugerido || '',
        costo: det.costo != null ? String(det.costo) : '',
        fecha_programada: det.fecha_programada || '',
        fecha_real: det.fecha_real || '',
        duration_hours: det.duration_hours != null ? String(det.duration_hours) : '',
        desarrollo_personal: !!det.desarrollo_personal,
        habilidades_blandas: !!det.habilidades_blandas,
        prevencion_riesgos: !!det.prevencion_riesgos,
        habilidades_tecnicas: !!det.habilidades_tecnicas,
        selEmps: new Set(empIds),
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    for (const tab of tabs) {
      ...
      try {
        ...
        const response = await fetch('/detecciones/data', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            id: activeEditingId || null,
            ...
            employee_ids: Array.from(tab.selEmps),
            linked_course_id: tab.linkedCourseId || null,
          }),
          credentials: 'include',
        });
        ...
        if (!response.ok) {
          ...
          throw new Error(json.error || 'Error al guardar la detección');
        }
